...
public class KafkaConsumerService {
    ...
    public void listenProductUploadEvents(ProductUploadEvent event) {
        ...
        logger.info(
                "Received Product Upload Event from Kafka (Batch ID: {}): {}",
                event.getBatchId(),
                event.getName()
        );
        ...
        try {
            if (productService.getProductByName(event.getName()).isPresent()) {
                logger.warn(
                        "SKIPPED (Duplicate): Product with name '{}' already exists. Batch ID: {}",
                        event.getName(),
                        event.getBatchId()
                );
                return;
            ...
            logger.info(
                    "SUCCESSFULLY ADDED: Product '{}' saved to DB. Batch ID: {}",
                    event.getName(),
                    event.getBatchId()
            );
            ...
            logger.error(
                    "Category not found for product '{}'. Batch ID: {}",
                    event.getName(),
                    event.getBatchId(),
                    e
            );
            ...
            logger.error(
                    "Failed to process product '{}'. Batch ID: {}",
                    event.getName(),
                    event.getBatchId(),
                    e
            );
